/**
 * @license React
 * react-server-dom-esm-client.node.production.min.js
 *
...
function prepareDestinationForModuleImpl(moduleLoading, // Chunks are double-indexed [..., idx, filenamex, idy, filenamey, ...]
mod, nonce) {
  if (typeof moduleLoading === 'string') {
    preinitModuleForSSR(moduleLoading + mod, nonce, undefined);
  } else if (moduleLoading !== null) {
    preinitModuleForSSR(moduleLoading.prefix + mod, nonce, moduleLoading.crossOrigin);
  }
}
...
function createFromNodeStream(stream, moduleRootPath, moduleBaseURL, options) {
  const response = createResponse(moduleRootPath, moduleBaseURL, noServerCall, options ? options.encodeFormAction : undefined, options && typeof options.nonce === 'string' ? options.nonce : undefined, undefined // TODO: If encodeReply is supported, this should support temporaryReferences
  );
